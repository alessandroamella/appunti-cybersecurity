...
\usepackage{hyperref}
\hypersetup{
    pdftitle={Appunti di Probabilità Discreta e Teoria dei Numeri per Cybersecurity},
...
\usepackage{xcolor}
\definecolor{bgcolor}{RGB}{30,30,30} % Sfondo grigio scuro
\definecolor{maintext}{RGB}{230,230,230} % Testo bianco sporco
\definecolor{headercolor}{RGB}{100,180,255} % Blu chiaro per titoli
\definecolor{codebg}{RGB}{40,40,40} % Sfondo per blocchi di codice
...
\setminted{
    ...
    bgcolor=codebg % Sfondo personalizzato per minted
}
\newminted{text}{bgcolor=codebg} % Per testo semplice con sfondo

% --- Stile TikZ per diagrammi su sfondo scuro ---
\tikzstyle{block} = [rectangle, draw=cyan, fill=bgcolor!80!maintext,
    text width=8em, text centered, rounded corners, minimum height=3em, line width=0.5pt]
\tikzstyle{diamond_block} = [diamond, draw=cyan, fill=bgcolor!80!maintext,
    text width=6em, text centered, minimum height=2.5em, aspect=2, line width=0.5pt, inner sep=0pt]
\tikzstyle{line} = [draw=maintext, -Latex, line width=0.5pt]
\tikzstyle{startend} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm,text centered, draw=cyan, fill=bgcolor!60!maintext, line width=0.5pt]
