...
\begin{itemize}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[node distance=2cm]
            ...
            \node[above=0.5cm of alice] (msg) {$m$};
            
            % Encryption
            \node[block, below=1.5cm of alice] (encrypt) {$\oplus$};
            \node[left=1cm of encrypt] (key1) {$k$};
            
            % Ciphertext
            \node[right=3cm of encrypt] (cipher) {$c = m \oplus k$};
            
            % Mallory (attacker)
            \node[attacker, below=1cm of cipher] (mallory) {Mallory};
            
            % Modified ciphertext
            \node[right=2cm of mallory] (modified) {$c^* = c \oplus p_{diff}$};
            
            % Bob
            \node[user, right=4cm of cipher] (bob) {Bob};
            
            % Decryption
            \node[block, below=1.5cm of bob] (decrypt) {$\oplus$};
            \node[right=1cm of decrypt] (key2) {$k$};
            
            % Result
            \node[below=1cm of decrypt] (result) {$m^* = m \oplus p_{diff}$};
            ...
            % Labels
            \node[below=2.5cm of encrypt, text width=8cm, align=center] {
