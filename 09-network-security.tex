...
\begin{tikzpicture}[node distance=0.5cm and 2cm]
    ...
    \node[attacker, below right=0.5cm and -1cm of c_estab, text width=3cm, font=\tiny] (attack_note)
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=0.8, transform shape]
        \node (source) {Source (Alice)};
        \node[neutralnode, label={[white]above:Relay A Key}, minimum width=3cm, minimum height=2.5cm, right=1cm of source, fill=blue!60!black, opacity=0.8] (layerA) {};
        \node[neutralnode, label={[white]above:Relay B Key}, minimum width=2.2cm, minimum height=1.8cm, fill=green!60!black, opacity=0.8] (layerB) at (layerA.center) {};
        \node[neutralnode, label={[white]above:Relay C Key}, minimum width=1.4cm, minimum height=1.1cm, fill=red!60!black, opacity=0.8] (layerC) at (layerB.center) {};
        \node[font=\tiny, fill=white, text=black, inner sep=1pt] (msg) at (layerC.center) {Messaggio};

        \node[below=0.8cm of layerA.south, xshift=-0.7cm] (RA) {Relay A};
        \node[below=0.8cm of layerB.south, xshift=0cm] (RB) {Relay B};
        \node[below=0.8cm of layerC.south, xshift=0.7cm] (RC) {Relay C};
        \node[right=1cm of layerA] (dest) {Destinazione};

        \draw[myarrow] (source.east) -- (layerA.west);
        \draw[myarrow] ($(layerA.east)+(0,0.3cm)$) -- node[above,font=\tiny]{A decifra} ($(dest.west)+(0,0.3cm)$);
        \draw[myarrow] ($(layerA.east)+(0,0cm)$) -- node[above,font=\tiny]{B decifra} ($(dest.west)+(0,0cm)$);
        \draw[myarrow] ($(layerA.east)+(0,-0.3cm)$) -- node[above,font=\tiny]{C decifra} ($(dest.west)+(0,-0.3cm)$);

        \node[text width=4cm, font=\tiny, anchor=west, below right=0.1cm and 0.1cm of source.south east]
